...
class TLClassifier(object):
    def __init__(self, model_dir = None):
        ...
        detection_model_path = 'light_classification/tf_graphs/ssd_mobilenet_v1_coco_2018_11_30/frozen_inference_graph.pb'
        if not os.path.exists(detection_model_path):
            rospy.logerr('Detection model not found at {}'.format(detection_model_path))
        classification_model_path = 'light_classification/tf_graphs/classification/SqueezenetV1.pb'
	...
	self.im_height, self.im_width, _ = image.shape
        ...
        if box == None:
            rospy.loginfo("get_classification No BB")
            return box, TrafficLight.UNKNOWN
        left, right, top, bottom = box
        rospy.loginfo("get_classification found BB {} {} : {} {}".format(left, right, top, bottom))
        img_crop = image[top:bottom, left:right]
        traffic_light = cv2.resize(img_crop, (32, 32))
        return box, self.classification(traffic_light)
    ...
    def detection(self, image):
        ...
        with self.detection_session_tf.as_default(), self.graph_detection.as_default(): 
            ...
            return self.non_maximal_suppression(bounding_boxes, detection_confidence_scores, classes)

    def non_maximal_suppression(self,boxes, confidence_scores, classes):
	confidence = 0.1
	max_conf = 0
	traffic_class = 10
	box_id = -1
